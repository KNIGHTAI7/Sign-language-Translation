...
# ── Removed pytorch_lightning dependency ──
# App only needs the model class for inference
# pytorch_lightning is only needed during training
...
class SignLanguageLSTM(nn.Module):
    ...
    Inference-only version (no pytorch_lightning dependency).
    ...
    def __init__(
        ...
        learning_rate = 3e-4,  # unused at inference
        nhead         = 4
    ):
        super().__init__()
        d_model = hidden_size
...
# ── Keep these for training script compatibility ───────────────
# The training script imports this file and uses LightningModule
# We conditionally inherit based on availability
try:
    import pytorch_lightning as pl
    from torchmetrics import Accuracy
    import numpy as np

    class SignLanguageLSTMTrainer(pl.LightningModule):
        """Training wrapper — only used during training, not in app."""
        def __init__(self, input_size=243, hidden_size=256, num_layers=4,
                     num_classes=20, dropout=0.2, learning_rate=3e-4, nhead=4):
            super().__init__()
            self.save_hyperparameters()
            self.learning_rate = learning_rate
            self.model = SignLanguageLSTM(input_size, hidden_size, num_layers,
                                          num_classes, dropout, learning_rate, nhead)
    ...
    pass  # pytorch_lightning not available — inference only mode
